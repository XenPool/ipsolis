...
> **⚠️ Non-binding English convenience translation.** Only the German-language version of these General Terms and Conditions is legally binding. In the event of any discrepancy or conflict between the German and English versions, the German version shall prevail.
...
> **(a) Free use:** Free use without a license file is limited to (i) purely private use by natural persons outside any commercial, professional, governmental or other organizational activity, (ii) time-limited evaluation for up to 30 days, (iii) non-productive use for research and teaching purposes at state-recognized educational and research institutions, and (iv) productive use with up to **25 Active Users** (the "Free Tier") – including in a commercial, governmental or other organizational context. An "Active User" is a distinct natural person, identified by their email address, who at a given point in time holds at least one active asset request or assignment in the software; persons whose requests and assignments have all ended (e.g. cancelled, expired, revoked, returned) are not counted. Such use is free of charge. Legal basis: contract of gift (sec. 516 BGB).

> **(b) Commercial (paid) use:** Any use beyond lit. (a) requires a valid paid license file. In particular, any productive or operational use of the software **with more than 25 Active Users** in the context of a commercial, freelance, governmental or other organizational activity is commercial – irrespective of any intention to generate profit. The Free Tier under lit. (a)(iv) does not by itself constitute commercial use. Use with more than 25 Active Users expressly includes use by legal entities under public law (e.g. cities, municipalities, public authorities, public-law corporations and institutions) as well as by tax-privileged or charitable (*gemeinnützig*) organizations. Commercial licenses are offered in volume bands by number of Active Users. Legal basis: software lease agreement (secs. 535 et seq. BGB).

**2.3** The subject matter of commercial licensing is the temporary provision of the licensed software for commercial use within the documented functional scope, unlocked by a cryptographically signed license file (the "License File" or ".lic file"). The License File defines the scope of the acquired license (term, licensed number of users / volume band – i.e. the maximum number of Active Users permitted, where a band of "0" means unlimited –, enabled features).
...
**4.4** Expiry of the License File: After a time-limited License File expires, the Provider grants a grace period of 30 days during which the software may continue to be used in production at the previous license scope (in particular with the previous number of users). After this period, the software automatically switches to the free mode (Free Tier under sec. 2.2 (a); productive use with up to 25 Active Users). Even in this mode, the customer retains access to its own data stored in the operating environment and may export it. Where usage exceeds the Free-Tier limit, only the creation of access for additional new users is prevented (sec. 2.2); existing operation continues. The Provider is not liable for damage or business interruptions arising from the automatic switch to the free mode after license expiry. The customer is obliged to order a renewal in good time before expiry.
