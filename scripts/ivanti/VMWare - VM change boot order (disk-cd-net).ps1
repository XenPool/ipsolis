# VMWare - VM change boot order (disk-cd-net)
# Changes the boot order of a VM to: Hard Disk → CD-ROM → Network
...
Write-Log "=== Starting VMware Boot Order Change Script (Disk-CD-Net) ===" 'INFO'
...
$config = @{
    ...
    # Boot order settings
    BootOrder = @{
        # Desired boot sequence: Hard Disk → CD-ROM → Network
        Sequence = @('HardDisk', 'CD-ROM', 'Network')
    }
...
function New-BootOrderConfiguration {
    <#
    .SYNOPSIS
    Creates boot order configuration with Hard Disk, CD-ROM, and Network
    ...
    try {
        ...
        # Set boot order: Hard Disk → CD-ROM → Network
        $spec.BootOptions.BootOrder = @($disk, $cdrom, $network)
        Write-Log "Boot order set to: Hard Disk → CD-ROM → Network" 'SUCCESS'
...
function Test-VMBootOrder {
    ...
    try {
        ...
        # Expected order: Hard Disk, CD-ROM, Network
        $expectedTypes = @('VirtualMachineBootOptionsBootableDiskDevice',
        ...
        for ($i = 0; $i -lt [Math]::Min($bootOrder.Count, $expectedTypes.Count); $i++) {
            $actualType = $bootOrder[$i].GetType().Name
            $expectedType = $expectedTypes[$i] -replace 'VMware.Vim.', ''
            $deviceName = $actualType -replace 'VirtualMachineBootOptionsBootable', '' -replace 'Device', ''
            
            if ($actualType -eq $expectedType) {
                Write-Log "Position $($i + 1): ${deviceName} - Correct" 'SUCCESS'
            } else {
                Write-Log "Position $($i + 1): ${deviceName} - Incorrect (expected: $($expectedType -replace 'VirtualMachineBootOptionsBootable', '' -replace 'Device', ''))" 'WARNING'
                $allCorrect = $false
            }
        }
...
try {
    ...
    if ($verifySuccess) {
        ...
        Write-Log "New boot order: Hard Disk → CD-ROM → Network" 'SUCCESS'
        exit 0
    } else {
