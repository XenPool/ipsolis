...
- **Full audit attribution** — every audit row records *who* drove the change, not just *what* happened. Admin endpoints record the credential (`token:<name>` / `admin:session:<user>` / `admin:legacy_key`); portal mutations record the Entra-resolved user (`portal:user:<email>`) or `portal:anonymous` when SSO is disabled; signed-token approvals record `api:approval_token (approver:<email>)`. The same attribution flows into the order, asset, asset-type, approval, and delegation audit rows so any change can be traced back to the specific credential that triggered it
